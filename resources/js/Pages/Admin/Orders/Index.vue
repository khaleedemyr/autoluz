...
<template>
    <AdminLayout title="Pesanan Toko">
                ...
                <tbody>
                    <tr v-for="order in orders.data" :key="order.id" class="border-t border-black/5">
                        ...
                        <td class="px-4 py-3 text-right">
                            <Link :href="route('admin.orders.show', order.id)" class="text-xs font-semibold uppercase tracking-[0.12em] text-brand">Detail</Link>
                        </td>
                    </tr>
                    <tr v-if="!orders.data?.length">
                        <td colspan="6" class="px-4 py-10 text-center text-sm text-neutral-500">Belum ada pesanan.</td>
                    </tr>
                </tbody>
            </table>

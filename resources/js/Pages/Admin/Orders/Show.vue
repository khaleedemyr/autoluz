...
<template>
    <AdminLayout :title="`Pesanan ${order.number}`">
        <Head :title="`Pesanan ${order.number}`" />

        <div class="mb-4">
            <Link :href="route('admin.orders.index')" class="text-sm text-neutral-500 hover:text-brand">← Daftar pesanan</Link>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            <aside class="space-y-4">
                ...
                <form class="rounded-2xl border border-black/5 bg-white p-5 shadow-sm" @submit.prevent="submit">
                    ...
                    <button type="submit" class="mt-4 w-full rounded-full bg-brand px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-white" :disabled="form.processing">
                        Simpan
                    </button>
                </form>
            </aside>
